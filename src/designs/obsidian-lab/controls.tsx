/**
 * Obsidian presentation controls — ported from obsidian-incubation HEAD
 * fc22e6c7db7da05b6166e2f126c5e6c9e6a20223 (src/controls.tsx). Radix UI +
 * Lucide, pre-approved by the T12 dependency policy. Presentation-owned.
 */
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button className={s.menuTrigger} aria-label={label}>{trigger}</button>
      </DropdownMenu.Trigger>
      <DropdownMenu.Portal>
        <DropdownMenu.Content className={s.menu} align="end" sideOffset={10}>
          <DropdownMenu.Label className={s.menuLabel}>{label}</DropdownMenu.Label>
          {items.map((item) =>
            item.href && !onAction && !item.disabled ? (
              <DropdownMenu.Item key={item.key} asChild className={s.menuItem}>
                <a href={item.href}>{item.label}<ArrowUpRight size={14} /></a>
              </DropdownMenu.Item>
            ) : (
              <DropdownMenu.Item
                key={item.key}
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content className={s.menu} align="start" sideOffset={8}>
          ...
          <DropdownMenu.RadioGroup value={value} onValueChange={onChange}>
            {choices.map((choice) => (
              <DropdownMenu.RadioItem className={s.menuItem} key={choice.value} value={choice.value}>
                {choice.label}
  ...
  return (
    <Tooltip.Provider delayDuration={350}>
      <Tooltip.Root>
        ...
        <Tooltip.Portal>
          <Tooltip.Content className={s.tooltip} sideOffset={7}>{label}</Tooltip.Content>
        </Tooltip.Portal>
      </Tooltip.Root>
    </Tooltip.Provider>
  )
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        >
          ...
          {children}
          <Dialog.Close className={s.dialogClose} aria-label="Close dialog"><X size={20} /></Dialog.Close>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )
