...
/**
 * Department detail extension. The frozen detail view is intentionally not
 * mounted here: DepartmentPageModel supplies membership, not reports-to
 * relationships, so the chart would require invented parent edges.
 */
export function DepartmentAdapter({ model }: LabPageProps<DepartmentPageModel, ObsidianConfig>) {
  return (
    <div className={s.publicPage}>
      <a href={`${model.baseUrl}/departments`} className={s.backLink}>
        <ArrowLeft size={15} /> All departments
      </a>
      <div className={s.departmentTabs} role="tablist" aria-label="Department destinations">
        {model.destinations.map((destination) => (
          <a key={destination.href} href={destination.href} role="tab" aria-selected="false">
            {destination.label}
          </a>
        ))}
      </div>
      ...
      {model.manageHref && <a href={model.manageHref} className={s.quietLink}>Manage departments <ArrowUpRight size={14} /></a>}
    </div>
  )
}
